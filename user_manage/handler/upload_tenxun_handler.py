...
from MyWebApp.json_utils import is_empty
...
def upload_tenxun_file(request):
    try:
        ...
        last_name = str(int(t)) + os.path.splitext(file.name)[0][-10:].replace(',', '')
        file_name = "WebApp/" + str(file_dir) + "/" + last_name + os.path.splitext(file.name)[1]
        buf = BytesIO()
